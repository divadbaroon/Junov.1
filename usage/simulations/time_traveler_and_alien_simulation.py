...
def time_traveler_and_alien_simulation():
    ...
    -------------------------------------------------------------------------------------------------
    .process() queries for and returns a response (Queries GPT with a given prompt and returns the response. Can also detect intent used for custom commands (stored in packages), however, that is not used in this example)                                    
    ...
    time_traveler = Juno(profile_name='time_traveler')
    alien = Juno(profile_name='alien')
    
    # manually starting the conversation with a beginning input 
    beginning_input = 'Hello! Where are you from?'
    ...
    # endless loop of both agents exchanging dialog 
    while True:
