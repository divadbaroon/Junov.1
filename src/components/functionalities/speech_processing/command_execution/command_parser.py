...
class CommandParser:
	...
	def __init__(self, api_keys: dict, speech_verbalizer:str, intents_json, bot_settings:object):
	...
	def parse_commands(self, speech:str):
		...
		if not self.gpt_response and self.language != 'english' and top_intent != 'Translate_Speech':
			response = TranslateSpeech().translate_speech(speech_to_translate=response, language_from='english', language_to=self.language, translator_key=self.api_keys['translator_key'])
