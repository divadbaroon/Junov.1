...
class BotBehavior:
	...
	def _reconfigure_voice(self, new_property:str) -> None:
		...
		if new_property in ['male', 'female']:
			new_voice_name = self.voice_settings.retrieve_voice_name(new_property.lower())
		# update voice name in settings
		self._update_voice_name(new_voice_name)

	def _update_voice_name(self, new_voice_name:str) -> None:
		"""
		Updates the voice name in master_settings.json and set the reconfigure_verbalizer flag to True.
		"""
